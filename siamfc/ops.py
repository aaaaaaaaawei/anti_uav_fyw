...
def init_weights(model, gain=1):#初始化时的增益，默认为1
    for m in model.modules():
        if isinstance(m, nn.Conv2d):#使用 Xavier 均匀分布进行权重初始化，并将偏置项初始化为常数0。
            nn.init.xavier_uniform_(m.weight, gain)
            if m.bias is not None:
                nn.init.constant_(m.bias, 0)#常数初始化
        elif isinstance(m, nn.BatchNorm2d):#权重初始化为常数1，偏置项初始化为常数0
            nn.init.constant_(m.weight, 1)
            nn.init.constant_(m.bias, 0)
        elif isinstance(m, nn.Linear):#使用 Xavier 均匀分布进行权重初始化，并将偏置项初始化为常数0。
            nn.init.xavier_uniform_(m.weight, gain)
            if m.bias is not None:
                nn.init.constant_(m.bias, 0)
