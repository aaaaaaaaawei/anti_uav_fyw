...
class BalancedLoss(nn.Module):
    ...
    def forward(self, input, target):
        #pos_mask和neg_mask的shape和target（labels）相同，pos_mask在labels有值的位置用1填充
        #neg_mask在labels没值的位置用1填充
        pos_mask = (target == 1)
        neg_mask = (target == 0)
        pos_num = pos_mask.sum().float()
        neg_num = neg_mask.sum().float()
        weight = target.new_zeros(target.size())
        #将weight与pos_mask和neg_mask做映射，与pos_mask对应的部分用 1/pos_num填充，
        #与neg_mask对应的部分用1 / neg_num * self.neg_weight填充
